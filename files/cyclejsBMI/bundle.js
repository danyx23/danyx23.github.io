...
/***/ function(module, exports, __webpack_require__) {
	...
	var registerCustomElement = _require.registerCustomElement;
	...
	registerCustomElement("slider", function (rootElement$, props) {
	  var model = (function () {
	    var value$ = createStream(function (changeValue$, propsStartValue$) {
	      return propsStartValue$.merge(changeValue$);
	    });
	    var min$ = createStream(function (propsMin$) {
	      return propsMin$.shareReplay(1);
	    });
	    var max$ = createStream(function (propsMax$) {
	      return propsMax$.shareReplay(1);
	    });
	    var label$ = createStream(function (propsLabel$) {
	      return propsLabel$.startWith("").shareReplay(1);
	    });
	    return {
	      value$: value$,
	      min$: min$,
	  ...
	  var view = (function () {
	    var vtree$ = createStream(function (value$, min$, max$, label$) {
	      return Rx.Observable.combineLatest(value$, min$, max$, label$, function (value, min, max, label) {
	        return dom(
	          "div",
	          { "class": "form-group" },
	          dom(
	          ...
	          dom(
	            ...
	            dom("input", { className: "form-control", type: "range", value: value, min: min, max: max }),
	            dom(
	              "div",
	  ...
	  var intent = (function () {
	    var changeSlider$ = createStream(function (interactions$) {
	      return interactions$.choose("[type=range]", "input").map(function (event) {
	    ...
	    var changeInput$ = createStream(function (interactions$) {
	      return interactions$.choose("[type=text]", "input").map(function (event) {
	        return parseInt(event.target.value, 10);
	      }).filter(function (val) {
	  ...
	  return {
	    changeValue$: intent.changeValue$.tap(function (x) {
	      return console.log("slider changed to: " + x);
	    })
	  };
	});
	
	var model = (function () {
	  var height$ = createStream(function (changeHeight$) {
	    return changeHeight$.startWith(175);
	  });
	  var mass$ = createStream(function (changeMass$) {
	...
	var view = (function () {
	  var vtree$ = createStream(function (height$, mass$) {
	    return Rx.Observable.combineLatest(height$, mass$, function (height, mass) {
	      return dom(
	        "div",
	        null,
	        dom(
	          "div",
	          null,
	          dom("slider", { className: "slider-height", label: "Height (in cm):", value: height, min: 130, max: 220, key: 1 }),
	          dom("slider", { className: "slider-mass", label: "Weight (in kg): ", value: mass, min: 25, max: 150, key: 2 })
	        ),
	        dom(
	          "div",
	          null,
	          "Your BMI is: ",
	          String(calculateBMI(height, mass))
	        )
	      );
	    });
	  });
	...
	var user = (function () {
	  var interactions$ = createStream(function (vtree$) {
	    return render(vtree$, ".app").interactions$;
	  });
	  return {
	...
	var intent = (function () {
	  var changeHeight$ = createStream(function (interactions$) {
	    return interactions$.choose(".slider-height", "changeValue").map(function (event) {
	      return event.data;
	    });
	  });
	  var changeMass$ = createStream(function (interactions$) {
	    return interactions$.choose(".slider-mass", "changeValue").map(function (event) {
	      return event.data;
	    });
	  });
	  return {
